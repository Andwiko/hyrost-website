...
let cachedMojangNews = null;
let lastNewsFetch = 0;

exports.getMojangNews = async (req, res) => {
    try {
        const now = Date.now();
        if (cachedMojangNews && (now - lastNewsFetch < 30 * 60 * 1000)) {
            return res.json({ success: true, source: 'cache', data: cachedMojangNews });
        }

        const https = require('https');
        https.get('https://launchercontent.mojang.com/v2/news.json', (resp) => {
            let data = '';
            resp.on('data', (chunk) => { data += chunk; });
            resp.on('end', () => {
                try {
                    const parsed = JSON.parse(data);
                    const newsList = (parsed.entries || []).slice(0, 10).map(entry => ({
                        id: entry.id || entry.title,
                        title: entry.title,
                        category: entry.category || 'News',
                        tag: entry.tag || 'Minecraft',
                        date: entry.date,
                        text: entry.text,
                        readMoreLink: entry.readMoreLink,
                        cardImage: entry.cardImage ? `https://launchercontent.mojang.com/${entry.cardImage.url}` : null
                    }));
                    cachedMojangNews = newsList;
                    lastNewsFetch = Date.now();
                    return res.json({ success: true, source: 'live', data: newsList });
                } catch (e) {
        ...
        res.status(500).json({ success: false, message: "Gagal mengambil Mojang news", error: err.message });
    }
};
